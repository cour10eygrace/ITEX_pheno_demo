...
#raw values
plot(daring_phen_long$GDD~daring_phen_long$GDD_lag)
cor.test(daring_phen_long$GDD, daring_phen_long$GDD_lag)
...
check2<-group_by(flower_bud, species, trait2)%>%summarise(count=n_distinct(trait2))

save(daring_phen_long, flower_open, flower_bud, file="data/Mixed_mods_df.Rdata")
